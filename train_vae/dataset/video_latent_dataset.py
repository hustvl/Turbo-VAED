...
from .transform import ToTensorVideo, CenterCropVideo, RandomHorizontalFlipVideo, RandomCropVideo
...
class TrainVideoDataset(data.Dataset):
    ...
    def _make_dataset(self):
        ...
        if osp.exists(cache_file):
            ...
            for v in all_videos:
                latent_path = os.path.join(self.latent_folder, f"{os.path.splitext(os.path.basename(v))[0]}.pt")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            latents_path = os.path.join(self.latent_folder,f"{os.path.splitext(video_name)[0]}.pt")
            latents = torch.load(latents_path)
            return dict(video=video, latents=latents)
...
class ValidVideoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            latents_path = os.path.join(self.latent_folder,f"{os.path.splitext(video_name)[0]}.pt")
            latents = torch.load(latents_path)
            return {'video': real_video_tensor, 'latents': latents}
        except:
